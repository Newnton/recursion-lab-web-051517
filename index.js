...
function maxOf(array){
  if(array.length === 1){return array[0]}
    ...
    if(array[0] < array[array.length-1]){return maxOf(array.slice(1, array.length))}
    else{return maxOf(array.slice(0, array.length-1))}
  }
}

function includesNumber(array, number){
  if(array.length === 0){return false}
  else if(array[0] === number){return true}
